...
std::string get_calib_info(int type) {
    ...
    if (client.call(srv)) {
      ...
      ROS_ERROR("Failed to call service GetInfo , make sure you have launch mynteye device SDK nodelet");
      return "null";
    }
}
...
bool readMYNTConfig(std::string config_file) {
    auto img_intri = get_calib_info(8);
    ...
    if (img_intri != "null") {
        ...
        ROS_WARN("1. the mynteye s device ROS nodelet not been launched");
        ROS_WARN("2. the mynteye s device SDK version may be too old");
        ROS_WARN("3. the device calib data may not correct");
        return false;
    }

    auto img_extri = get_calib_info(9);
