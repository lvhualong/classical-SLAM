...
class Estimator
{
  public:
    ...
    Vector3d        Ps[(WINDOW_SIZE + 1)]; //Body --> world 平移
    Vector3d        Vs[(WINDOW_SIZE + 1)]; //
    Matrix3d        Rs[(WINDOW_SIZE + 1)]; //Body --> world 旋转
    ...
    IntegrationBase *pre_integrations[(WINDOW_SIZE + 1)]; //pre_integrations是一个数组，每一个元素是一个IntegrationBase类型的指针，对应每帧图像的预积分
    Vector3d acc_0, gyr_0; //first imu的数据

    //每两帧图像间的IMU 数据vector 数组
    vector<double> dt_buf[(WINDOW_SIZE + 1)];
    vector<Vector3d> linear_acceleration_buf[(WINDOW_SIZE + 1)];
    vector<Vector3d> angular_velocity_buf[(WINDOW_SIZE + 1)];

    int frame_count; //在窗口中的位置 0-10  0-~9是窗口 最后一个10是当前帧
