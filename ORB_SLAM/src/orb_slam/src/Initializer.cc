...
//创建一个SLAM初始化实例， 获取关键的图像和关键点，以及Ransac参数
Initializer::Initializer(const Frame &ReferenceFrame, float sigma, int iterations)
{
    mK = ReferenceFrame.mK.clone(); //内参

    mvKeys1 = ReferenceFrame.mvKeysUn; //特征提取的关键点
...
/*
***************************************
Initializer::Initialize 根据参考帧，当前帧和匹配点来算 R21 t21 P3D Trianglated

- 调用 FindHomography、FindFundamental矩阵，求H F
- 调用 ReconstructH , ReconstructF 来求 R21 t21 P3D Trianglated
***************************************
*/
                             ...
                             cv::Mat &R21, cv::Mat &t21, vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    mvKeys2 = CurrentFrame.mvKeysUn; //当前帧的关键点
    ...
    const int N = mvMatches12.size(); //匹配点的数据

    // Indices for minimum set selection
    vector<size_t> vAllIndices;
    vAllIndices.reserve(N); //避免重新分配
    ...
    // 为每个RANSAC 随机选取生成8个点的集合，用于本质矩阵求解
    ...
    // 开启两个线程分别用于计算fundamental matrix和homography
    vector<bool> vbMatchesInliersH, vbMatchesInliersF;
    float SH, SF; //两个矩阵的得分
    //计算每个点对的symmetric transfer errors，和卡方分布的对应值比较，由此判定该点是否为内点。累计内点的总得分
    cv::Mat H, F;
    ...
    float RH = SH / (SH + SF);
    //如果SH / (SH + SF) > 0.4 说明大多数点都在一个平面，选择homography矩阵， 否则选择fundamental计算
    // Try to reconstruct from homography or fundamental depending on the ratio (0.40-0.45)
    if (RH > 0.40)
    {
        ROS_INFO_STREAM( "Initialization >> choose H matrix (Planar)");
        return ReconstructH(vbMatchesInliersH, H, mK, R21, t21, vP3D, vbTriangulated, 1.0, 50);
    }
    else //if(pF_HF>0.6)
    {
        ROS_INFO_STREAM( "Initialization >> choose F matrix (non-planar)");
        return ReconstructF(vbMatchesInliersF, F, mK, R21, t21, vP3D, vbTriangulated, 1.0, 50);
    }
...
/*
*********************************************
Initializer::FindHomography 根据匹配的内点来计算 vbMatchesInliers，socre H21

- 调用 Normalize， ComputeH21，CheckHomography
*********************************************
*/
...
/*
*********************************************
Initializer::FindFundamental 根据匹配的内点来计算 vbMatchesInliers，socre F21

- 调用 Normalize， ComputeF21，CheckHomography
*********************************************
*/
...
// 从特征点匹配求homography矩阵H（normalized DLT）
...
// IV. AUTOMATIC MAP INITIALIZATION in Author's paper
...
输入param@   vbMatchesInliers矩阵内点  H21 K矩阵
输出param@   R21/t21/ P3D vbTriangulated
限制param@   minParallax, minTriangulated
******************************
*/
...
// 均值归一化特征点到同一尺度（作为normalize DLT的输入）
